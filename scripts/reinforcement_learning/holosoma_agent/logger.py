...
class DictBenchmarkLogger:
    def __init__(
        self, log_dir: str, tag: str, num_envs: int, max_trials: int, max_episode_length: int, log_item: list[str]
    ):
            ...
            for item, data in item_dict.items():
                if data is not None:
                    ...
                    if len(self.item_log_cache[item][idx]) == 0:
                        ...
                        self.item_log_cache[item][idx] = np.concatenate(
                            [self.item_log_cache[item][idx], np.expand_dims(data[idx], axis=0)], axis=0
                        )
    ...
    def fill_missing_data(self, data):
        ...
        if num_dim == 1:
            ...
            return np.concatenate(
                [data, np.tile(np.zeros_like(data[0:1]), (self.max_episode_length - len(data), 1))], axis=0
            )
        elif num_dim == 3:
            return np.concatenate(
                [data, np.tile(np.zeros_like(data[0:1]), (self.max_episode_length - len(data), 1, 1))], axis=0
            )
        else:
