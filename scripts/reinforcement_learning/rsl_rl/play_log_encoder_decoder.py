...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            bad_orientation_idx = termination_manager._term_name_to_term_idx.get("bad_orientation", None)
            ...
            bad_orientation = term_dones[:, bad_orientation_idx] if bad_orientation_idx is not None else None
                ...
                if done_flag == 1:
                    ...
                    if bad_orientation is not None:
                        if bad_orientation[i]:
                            print(
                                f"[INFO] Env {i}: Episode {episode_counter[i]} - Bad orientation with length {episode_length_log[i]}"
                            )
