...
"""
# TODO: support recurrent and encoder policies
# Right now, only MLP is supported.
...
class _TorchPolicyExporter(torch.nn.Module):
    ...
    def __init__(self, policy, normalizer=None):
        super().__init__()
        if hasattr(policy, "is_recurrent"):
...
class _OnnxPolicyExporter(torch.nn.Module):
    ...
    def export(self, path, filename):
        ...
        if self.is_recurrent:
            ...
            first_layer = next(m for m in self.actor.modules() if isinstance(m, torch.nn.Linear))
            obs = torch.zeros(1, first_layer.in_features)
            torch.onnx.export(
                self,
                obs,
                os.path.join(path, filename),
