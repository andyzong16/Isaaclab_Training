...
def terrain_material_parameters(
    ...
    # extract the used quantities (to enable type-hinting)
    # fake values on rigid ground
    friction_coef = 1.0 * torch.ones(env.num_envs, device=env.device)
    rho_c = 1.0 * torch.ones(env.num_envs, device=env.device)
    mu_int = 1.0 * torch.ones(env.num_envs, device=env.device)
