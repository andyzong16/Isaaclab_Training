...
class UniformVelocityYawCommand(UniformVelocityCommand):
    ...
    def _update_metrics(self):
        ...
        self.metrics["error_yaw"] += (
            torch.abs(
                math_utils.quat_error_magnitude(math_utils.yaw_quat(self.robot.data.root_quat_w), self.heading_target)
            )
            / max_command_step
        )
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        self.heading_target[env_ids] = math_utils.yaw_quat(self.robot.data.root_quat_w[env_ids])
    ...
    def _update_command(self):
        ...
        current_heading = math_utils.yaw_quat(self.robot.data.root_quat_w)
    ...
    def _debug_vis_callback(self, event):
        ...
        self.goal_heading_visualizer.visualize(
            heading_pos_w,
            self.heading_target,
            torch.tensor([6.0, 1.0, 0.1], device=self.device).repeat(self.num_envs, 1),
        )
