...
class SpringDamper:
    ...
    def randomize_ground_stiffness(self, env_ids: torch.Tensor, k: torch.Tensor) -> None:
        ...
        wp.launch(
            kernel=update_array_with_index,
            dim=len(env_ids),
            inputs=[wp.from_torch(env_ids, dtype=wp.int64), wp.from_torch(k, dtype=wp.float32), self.k],
            device=self.device,
        )
    ...
    def update_material_density(
        ...
        wp.launch(
            kernel=update_array_with_index,
            dim=len(env_ids),
            inputs=[wp.from_torch(env_ids, dtype=wp.int64), wp.from_torch(b_new, dtype=wp.float32), self.b],
            device=self.device,
        )
